...
student = (name = "Gurmeet Singh", kerberos_id = "gurmeet")
...
# ╔═╡ f8c71030-4824-11eb-0349-838942be4bb3
c = RGBX(0.19, 0.39, 0.49)

# ╔═╡ 0b98c310-4826-11eb-023e-15b666cd1545
quantize(c)
...
The image is unrecognisable with intensity around 3. It required higher than 1 as even with high error rate the correlation between nearby pixels can be observed. Prediction: decimated image will become unrecognizable at a smaller rate though still greater than 1.
"""
...
# ╔═╡ 1a228bb0-482a-11eb-3e37-2df7606c125c
noisify(decimate(philip, 3), philip_noise)
...
function blur_1D(v, l)
	s = length(v)
	ev = [extend(v, i) for i in (1-l):(s+l)]
	return [mean(ev[i:(i+2*l)]) for i in 1:s]

	## Deba's code for testing
	# vb = zeros(length(v))
	# for i in 1:length(v)
	# 	vb[i] = mean([extend(v, i+j) for j in -l:l])
	# end
	# return vb
...
# ╔═╡ 60efb5f0-482e-11eb-2366-fb9db8cb2953
vcat(colored_line(v), colored_line(blur_1D(v, l_box)))
...
function convolve_vector(v, k)
	...
	l = (length(k)-1)÷2
	ev = [extend(v, i) for i in (1-l):(s+l)]
	return [sum(ev[i:(i+2*l)].* reverse(k)) for i in 1:s]
end
...
function gaussian_kernel(n)
	...
	m = [g(i-(n+1)/2) for i in 1:n]
	
	return m./sum(m)
end

# ╔═╡ 5008a2be-4832-11eb-01aa-a188299d9c4e
gaussian_kernel(7)
...
@bind gaussian_kernel_size_1D Slider(1:2:30, show_value=true) # change this value, or turn me into a slider!
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	e_M = [extend_mat(M, i, j) for i in (1-li):(si+li), j in (1-lj):(sj+lj)]
	r_K = reverse(reverse(K, dims=1), dims=2)
	
	return [sum(e_M[i:(i+2*li), j:(j+2*lj)].* r_K) for i in 1:si, j in 1:sj]
...
function with_gaussian_blur(image)
	...
	g(x, y) = exp(-(x^2+y^2)/2) / sqrt(2 * pi)
	
	n = 7
	
	m = [g(i-(n+1)/2, j-(n+1)/2) for i in 1:n, j in 1:n]
...
function with_sobel_edge_detect(image)
	...
	G_x = convolve_image(image, k_x)
	G_y = convolve_image(image, k_y)
	
	return Gray.(sqrt.(G_x.*G_x .+ G_y.*G_y))
end
